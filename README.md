...
[important] we ask you **not to change** these functions and their signatures. First because these functions are used by the CLI tool and the web server. Secondly, because your code might be automatically tested.

## Datatypes

### Transactions

Here is a simple example of transaction:
...
- it takes two addresses as inputs. These addresses are necessarily part of UTXOs from one or several past but yet confirmed transactions. The total of these two inputs UTXOs is necessarily equal to 110 dotcoins (the total of output UTXOs)
...
Like bitcoin the mining difficulty is about finding a nonce that makes the block id starts with a repetaing sequence of `1`. In the example above, the mining difficulty is 2 since the two block ids (`\_id` and `previous`) starts with two `1`s.

While working, you may want to set the difficulty to 1 or 2. However, the dotcoin server has a mining difficulty of 4.
...
Transaction ids and block ids are Keccak256 hashes encoded in Base58. The package [@noble/hashes](https://www.npmjs.com/package/@noble/hashes) provides primitives for `keccak_256` hashing. In `utils/utils.mjs`, we provide two methods `getTransactionHash` and `getBlockHash` that returns base58-encoded hash.
...
Signatures are also encoded in Base58. The @scure/bip32 package provide a primitive to sign decoded hash (`Uint8Array`). The output signature is also an `Uint8Array` that can be encode into Base58 using @scure/base package.

## Implementation

To implement the Dotcoin client and the server, we recommend **not to use** the CLI tool or the web server at first. Instead, write some unit tests on the core functions directly. As started code, we provide some unit tests in `test-core.mjs` that you can using `npm run test`.

### The Client Wallet

The easiest way to get you started is to implement the client. By the end of this task, you should be able to create a wallet, get an account balance, make a transaction and mine a block. To do those tasks you will need to write to generate signed transactions and valid blocks.

The client is essentially a system to manage BIP44 keys. Recalling from the course, BIP44 keys are derived from a BIP 39 mnemonic phrase and a BIP32 key derivation path.
...
For instance, when Alice wants to receive dotcoins from Bob on her account `0`, she gives Bob her receiving public key derive from `M/44'/1'/0'/1`. Then Bob uses that key to generate a new child key that will be used in a transaction as the UTXO address. Since that address must be unique, Bob tries to find an index, starting from index 0, that has not been used. Once he founds one, he uses it in his transaction.

Furthermore, let's assume that Bob wants to send 60 dotcoins from a UTXO input of 100. This means that Bobs needs to get 40 dotcoins back. For that he will generate an unused address derive from his change address derive. Assuming that Bob uses his 4th account, the path for the change parent is `M/44'/1'/4'/0`. Like before, bob must find an index, starting from 0, that has not been used to generate his change address.
...
- Command `create` generates a new wallet. It asks the user to enter a password, generate a mnemonic phrase and stores it securely in a wallet file `wallet.bin` by default (see command options). The wallet file contains the mnemonic phrase encrypted with the user's password.
...
- Command `address` returns the receiving public key for the given account. It extracts the mnemonic from the wallet file and compute the receiving address. Here is an example for account 0:
...
- Command `mine` mines a block and transfers the coinbase transaction reward to the give account. Here is an example for account 0:
...
- have a password bypass `--pasword`
